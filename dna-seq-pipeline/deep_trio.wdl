...
workflow DeepTrio {
    ...
    call files.copy as copy_deepvariant {
        input:
            files = [ go_deep.child_vcf, go_deep.child_gvcf,
                    go_deep.parent1_vcf, go_deep.parent1_gvcf,
                    go_deep.parent2_vcf, go_deep.parent2_gvcf,
                    go_deep.report ], destination = destination
    }

    output {
        File child_vcf = copy_deepvariant.out[0]
        File child_gvcf = copy_deepvariant.out[1]
        File parent1_vcf = copy_deepvariant.out[2]
        File parent1_gvcf = copy_deepvariant.out[3]
        File parent2_vcf = copy_deepvariant.out[4]
        File parent2_gvcf = copy_deepvariant.out[5]
        File report = copy_deepvariant.out[6]
    }
}
task go_deep{
    input {
    ...
    output {
        ...
        File report = "test.visual_report.html"
    }
}
